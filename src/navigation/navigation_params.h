...
// Local occupancy grid used to compute the obstacle-aware cost-to-go wavefront.
// The grid is centered on base_link (+x forward, +y left, origin at center).
struct GridParams {
  GridParams() : resolution(0.05), half_extent(3.5) {}

  float resolution;   // meters per cell
  float half_extent;  // half the grid side length, in meters
};
